...
var _ = Describe("User API", func() {
	...
	BeforeEach(func() {
		...
		userId := int(responseBodyCreateUser["data"].(map[string]interface{})["id_user"].(float64))
		...
		When("the fields are incorrect", func() {
			When("the name field is incorrect", func() {
				It("should return error required", func() {
					// Update User
					requestBody := strings.NewReader(`{"email": "widdy@gmail.com","address":"nganjok","phone":"082299","password": "Rahasia123"}`)
					request := httptest.NewRequest(http.MethodPatch, "/api/users/"+strconv.Itoa(rowUserId), requestBody)
					request.Header.Add("Content-Type", "application/json")
					request.Header.Add("X-API-KEY", configuration.XApiKey)
					request.AddCookie(cookie)
					request.Header.Set("Authorization", fmt.Sprintf("Bearer %v", tokenJWT))
	...
	Describe("Delete User /users/:id", func() {
		...
		When("user is found", func() {
			It("should return a successful delete user response", func() {
				// Update User
				request := httptest.NewRequest(http.MethodDelete, "/api/users/"+strconv.Itoa(rowUserId), nil)
				request.Header.Add("Content-Type", "application/json")
				request.Header.Add("X-API-KEY", configuration.XApiKey)
				request.AddCookie(cookie)
				request.Header.Set("Authorization", fmt.Sprintf("Bearer %v", tokenJWT))
	...
	Describe("Find ALl User /users", func() {
		...
		When("the user is present", func() {
			It("should return a successful and show all users", func() {
				// Create user
				requestBody := strings.NewReader(`{"name": "Widdy","email": "arfian@gmail.com","address":"nganjok","phone":"082299","password": "Rahasia123"}`)
				request := httptest.NewRequest(http.MethodPost, "/api/auth/register", requestBody)
				request.Header.Add("Content-Type", "application/json")
				request.Header.Add("X-API-KEY", configuration.XApiKey)
				...
				var responseBodyCreateUserOne map[string]interface{}
				_ = json.NewDecoder(writer.Result().Body).Decode(&responseBodyCreateUserOne)
				...
				var responseBodyCreateUserTwo map[string]interface{}
				_ = json.NewDecoder(writer.Result().Body).Decode(&responseBodyCreateUserTwo)
				...
				Expect(int64(responseBodyCreateUserOne["data"].(map[string]interface{})["id_user"].(float64))).To(Equal(int64(users[1].(map[string]interface{})["id_user"].(float64))))
				Expect(responseBodyCreateUserOne["data"].(map[string]interface{})["name"]).To(Equal(users[1].(map[string]interface{})["name"]))

				Expect(int64(responseBodyCreateUserTwo["data"].(map[string]interface{})["id_user"].(float64))).To(Equal(int64(users[0].(map[string]interface{})["id_user"].(float64))))
				Expect(responseBodyCreateUserTwo["data"].(map[string]interface{})["name"]).To(Equal(users[0].(map[string]interface{})["name"]))
			})
		})
	})
	...
	Describe("Find By Id User /users/:id", func() {
		...
		When("user is found", func() {
			It("should return a successful find user by id", func() {
				// Find By Id User
				request := httptest.NewRequest(http.MethodGet, "/api/users/"+strconv.Itoa(rowUserId), nil)
				request.Header.Add("Content-Type", "application/json")
				request.Header.Add("X-API-KEY", configuration.XApiKey)
				request.AddCookie(cookie)
				request.Header.Set("Authorization", fmt.Sprintf("Bearer %v", tokenJWT))
