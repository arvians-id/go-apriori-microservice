...
func TestFindByAll(t *testing.T) {
	...
	t.Run("when the list user is not found, then return error", func(t *testing.T) {
		...
		users, err := userService.FindAll(ctx, new(empty.Empty))
		assert.NotNil(t, err)
		assert.Nil(t, users)
		assert.Equal(t, err, errors.New(util.ErrorNotFound))
		test.Unset()
	})
	...
	t.Run("when the list user is found, then return list users", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, userRequests[0].Name, users.User[0].Name)
		assert.Equal(t, 2, len(users.User))
		test.Unset()
	})
}
...
func TestFindById(t *testing.T) {
	t.Run("when the user is not found, then return error", func(t *testing.T) {
		...
		ctx := context.Background()
		user, err := userService.FindById(ctx, &pb.GetUserByIdRequest{
			Id: 1,
		})
		assert.NotNil(t, err)
		assert.Nil(t, user)
	...
	t.Run("when the user is found, then return user details", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, userRequest.Name, user.User.Name)
		test.Unset()
	})
}
...
func TestFindByEmail(t *testing.T) {
	t.Run("when the user is not found, then return error", func(t *testing.T) {
		...
		ctx := context.Background()
		user, err := userService.FindByEmail(ctx, &pb.GetUserByEmailRequest{
...
func TestCreate(t *testing.T) {
	...
	ctx := context.Background()
	user, err := userService.Create(ctx, &pb.CreateUserRequest{
...
func TestUpdate(t *testing.T) {
	t.Run("when the user is not found, then return error", func(t *testing.T) {
		...
		ctx := context.Background()
		user, err := userService.Update(ctx, &pb.UpdateUserRequest{
			IdUser:  1,
			Role:    2,
			Name:    "Arfiansyah",
...
func TestDelete(t *testing.T) {
	t.Run("when the user is not found, then return error", func(t *testing.T) {
		...
		ctx := context.Background()
		_, err := userService.Delete(ctx, &pb.GetUserByIdRequest{
			Id: 1,
		})
		assert.NotNil(t, err)
		assert.Equal(t, err, errors.New(util.ErrorNotFound))
